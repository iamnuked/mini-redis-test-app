...
- 키별 자료형 검증과 자료구조 확장을 수용할 수 있도록 저장 모델을 일반화한다.
- `Hash`, `List`, `Set`, `Sorted Set` 연산을 현재 구조 안에서 수용할 수 있도록 명령 실행 경로를 확장한다.
...
- 본 시스템은 문자열 키를 사용하며, 각 키는 하나의 자료형이 지정된 값 엔트리를 가진다.
- 본 시스템은 `STRING`, `HASH`, `LIST`, `SET`, `ZSET` 자료형과 그 전용 명령을 지원한다.

단, 아키텍처는 현재 지원 자료구조 외에 추가 자료형과 세부 연산이 늘어나더라도 확장 가능해야 한다.
...
- 명령별 인자 수와 요청 인자 자료형 요구사항을 검증한다.
...
- 엔트리 저장소: `Map<String, ValueEntry>`
- 만료 저장소: `Map<String, Long>`

`ValueEntry` 예시:

- `type: STRING | HASH | LIST | SET | ZSET`
- `value: String | Map<String, String> | List<String> | Set<String> | SortedMemberCollection`
...
- 자료형별 세부 연산 로직은 Service 내부 또는 자료형 전용 하위 서비스로 캡슐화할 수 있으나, 네트워크 계층으로 새어 나가면 안 된다.
...
- 응답 기본형: Simple String, Blob String, Number, Null, Simple Error, Map, Array
...
- Validator 테스트: 인자 수와 수치 인자 형식 검증이 올바른지 확인
...
- `HSET` 후 `HGET`이 필드 값을 반환하는지 확인
- `LPUSH`/`RPUSH` 후 `LRANGE`가 순서를 유지하는지 확인
- `SADD` 후 `SMEMBERS`가 중복 없는 집합을 반환하는지 확인
- `ZADD` 후 `ZRANGE`가 점수 순으로 반환되는지 확인
- 자료형이 다른 키에 명령 적용 시 타입 오류가 반환되는지 확인
