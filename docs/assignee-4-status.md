...
- RESP 응답의 일부 타입에 대한 임시 파싱 및 렌더링
  ...
  - Error
  - Map
- 현재 서버 구현이 연결당 요청 1개를 처리하므로, CLI는 `HELLO 3` 확인과 실제 명령 전송을 별도 연결로 수행
- `python3 cmd/mini_redis_cli/main.py` 형태의 직접 실행을 위한 import 경로 보정

설명:

- 현재 CLI는 담당자 1의 공식 RESP 계층에 완전하게 붙지 못한 상태라 최소 동작 검증을 위해 CLI 내부에 임시 RESP 인코딩/디코딩 로직을 유지하고 있다.
- 다만 병합된 서버 코드가 현재 `연결 1개 -> 요청 1개 -> 응답 1개 -> 종료` 흐름으로 동작하므로, 이 제약에는 맞도록 `HELLO 3`과 실제 명령을 분리해 전송하도록 조정했다.
...
- CLI 단위 테스트 추가
  ...
  - 명령 누락 시 종료 코드
  - 연결 실패 시 종료 코드
  - `HELLO 3` 사전 확인 후 실제 명령 전송 순서
  - `HELLO 3` 오류 시 명령 전송 중단
  - 서버 오류 응답 처리
...
- 현재 서버 측 RESP 계층은 구현되었지만, CLI가 직접 재사용할 수 있는 응답 디코더는 아직 없다.
- 특히 현재 `request_decoder.py`는 요청 파싱 중심이고, CLI가 필요로 하는 Error, Null, Map 중심 응답 디코딩을 공용 계층에서 바로 재사용하기 어렵다.
- 현재 로컬 Python 3.9.6 환경에서는 프로토콜 계층 일부가 `typing.TypeAlias` 사용으로 바로 import되지 않는다.

구현을 위해 필요한 내용:

- 담당자 1과 CLI용 공용 RESP 응답 디코더를 둘지 여부를 합의해야 한다.
- 담당자 1이 Python 최소 버전 또는 타입 호환성 정책을 정리해야 한다.
...
- 현재 서버의 요청 처리 경로는 대부분 연결되었지만, 세션 처리 예외를 RESP 오류 응답으로 반환하지 않는 문제가 남아 있다.
- 서버 엔트리포인트는 여전히 직접 실행 편의성이 충분히 정리되지 않았다.
- 로컬 Python 3.9.6 환경에서 일부 프로토콜 코드가 바로 import되지 않아 실제 end-to-end 자동화가 막힌다.

구현을 위해 필요한 내용:

- 담당자 3이 세션 처리 오류 응답 경로를 정리해야 한다.
- 서버 직접 실행 경로를 안정적으로 맞출 수 있어야 한다.
- 팀 차원에서 Python 실행 버전 기준을 맞춰야 한다.
