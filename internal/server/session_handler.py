...
class SessionHandler:
    ...
    def _process_request(self, request_bytes: bytes) -> bytes:
        ...
        if protocol_result.has_immediate_response():
            if protocol_result.protocol_version is not None:
                self._session_context.protocol_version = protocol_result.protocol_version
            return self._response_encoder.encode(
                protocol_result.response,
                protocol_version=self._session_context.protocol_version,
            )
        ...
        self._apply_session_state(protocol_result.command)
        return self._response_encoder.encode(
            service_result,
            protocol_version=self._session_context.protocol_version,
        )

    def _apply_session_state(self, command) -> None:
        if command.name == "SELECT":
            self._session_context.selected_db = int(command.arguments[0])
            return

        if command.name == "CLIENT":
            subcommand = command.arguments[0].upper()
            if subcommand == "SETNAME":
                self._session_context.client_name = command.arguments[1]
            elif subcommand == "SETINFO":
                self._session_context.client_info[command.arguments[1]] = command.arguments[2]
